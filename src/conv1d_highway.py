...
class Conv1DHighway(_Conv):
    def __init__(self, filters,
                 ...
                 **kwargs):
        super(Conv1DHighway, self).__init__(
            rank=1,
            filters=filters,
    ...
    def get_config(self):
        config = super(Conv1DHighway, self).get_config()
        config.pop('rank')
        return config

    def build(self, input_shape):

        super(Conv1DHighway, self).build(input_shape)
    ...
    def get_output_shape_for(self, input_shape):
        super(Conv1DHighway, self).get_output_shape_for(input_shape)
